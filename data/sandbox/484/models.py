...
def setHyperparams(extractor):
    global TrainingEpochs, LearningRate, L2Alpha
    clsName = extractor.__class__.__name__
    print("feature extractor is a ", clsName)
    if clsName in ["UnigramFeatureExtractor", "CustomizedFeatureExtractor"]:
        TrainingEpochs = 25
        LearningRate = 0.1
        L2Alpha = 1e-5
    else:
...
TrainingEpochs = 25 # 400     # 25
LearningRate = 0.1  # 1.5     # 0.1
L2Alpha = 1e-5 # 0, 1e-6
...
class CustomizedFeatureExtractor(FeatureExtractor):
    def __init__(self, indexer: Indexer):
        ...
        excl = ["don", "nor", "down", "up", "no", "not", "against"]
        self.stopwords = stopwords - set(excl)
        self.neg = set(["don", "n't", "not", "no", "never"])
...
def train_lr(train_exs: List[SentimentExample], feat_extractor: FeatureExtractor) -> LogisticRegressionClassifier:
    setHyperparams(feat_extractor)
