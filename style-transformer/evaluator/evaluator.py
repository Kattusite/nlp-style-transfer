...
# Should read: csv.field_size_limit(maxInt)
...
class Evaluator(object):
    ...
    def __init__(self):
        ...
        # self.classifier_novels = keras.models.load_model(novels_acc_path)
...
###############################################################################
    ...
    def novels_style_check(self, text_transfered, style_origin):
    ...
    def novels_acc_b(self, texts, styles_origin):
        ...
        for text, style in zip(texts, styles_origin):
            if self.novels_style_check(text, style):
                count += 1
        return count / len(texts)

    def novels_acc_0(self, texts):
        styles_origin = [0] * len(texts)
        return self.novels_acc_b(texts, styles_origin)

    def novels_acc_1(self, texts):
        styles_origin = [1] * len(texts)
        return self.novels_acc_b(texts, styles_origin)

    def novels_ref_bleu_0(self, texts_neg2pos):
        assert len(texts_neg2pos) == 500, 'Size of input differs from human reference file(500)!'
        sum = 0
        n = 500
        for x, y in zip(self.novels_ref[0], texts_neg2pos):
            sum += self.nltk_bleu([x], y)
        return sum / n

    def novels_ref_bleu_1(self, texts_pos2neg):
        assert len(texts_pos2neg) == 500, 'Size of input differs from human reference file(500)!'
        sum = 0
        n = 500
        for x, y in zip(self.novels_ref[1], texts_pos2neg):
            sum += self.nltk_bleu([x], y)
        return sum / n

    def novels_ref_bleu(self, texts_neg2pos, texts_pos2neg):
        assert len(texts_neg2pos) == 500, 'Size of input differs from human reference file(500)!'
        assert len(texts_pos2neg) == 500, 'Size of input differs from human reference file(500)!'
        sum = 0
        n = 1000
    ...
    def novels_ppl(self, texts_transfered):
        ...
        for i, line in enumerate(texts_transfered):
            ...
            score = self.novels_ppl_model.score(line)
            sum += score
